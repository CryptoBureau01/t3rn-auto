...
def Executor(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        bridgeaddr = web3.to_checksum_address('0xF221750e52aA080835d2957F2Eed0d5d7dDD8C38') #for OP
        ...
        print(f'Processing Executor Bridge {bridgedmin} From ETH OP TO ETH OP')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bridge {bridgedmin} From ETH OP TO ETH OP Success!')
...
def Get_Estimate(sender, key, amount):
    ...
    data = {
        ...
        "toAsset": "eth",
        "fromChain": "opsp",
        "toChain": "opsp", #arbt (arb) | bssp (base) | blss (blast) | opsp (op)
...
def BidExecute(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        print(f'Processing Bid Executor Bridge {bridgedmin} From ETH OP TO ETH OP')
        web3_brn.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bid Bridge {bridgedmin} From ETH OP TO ETH OP Success!')
...
def BLAST_BLAST(sender, key, amount, amountin):
    try:
        ...
        codebridge = bytes.fromhex('6f707370') #OP>OP
        ...
        print(f'Processing Bridge {bridgedmin} ETH OP TO ETH OP')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Bridge {bridgedmin} ETH OP TO ETH OP Success!')
        print(f'TX-ID : {str(web3.to_hex(tx_hash))}')
