...
def Executor(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        bridgeaddr = web3.to_checksum_address('0x30A0155082629940d4bd9Cd41D6EF90876a0F1b5') #for BASE
        ...
        print(f'Processing Executor Bridge {bridgedmin} From ETH BASE TO ETH BASE')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bridge {bridgedmin} From ETH BASE TO ETH BASE Success!')
...
def Get_Estimate(sender, key, amount):
    ...
    data = {
        ...
        "toAsset": "eth",
        "fromChain": "bssp",
        "toChain": "bssp", #arbt (arb) | bssp (base) | blss (blast) | opsp (op)
...
def BidExecute(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        print(f'Processing Bid Executor Bridge {bridgedmin} From ETH BASE TO ETH BASE')
        web3_brn.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bid Bridge {bridgedmin} From ETH BASE TO ETH BASE Success!')
...
def BLAST_BLAST(sender, key, amount, amountin):
    try:
        ...
        codebridge = bytes.fromhex('62737370') #base>base
        #codebridge = bytes.fromhex('626c7373') #blast>blast
        amountzero = 0
        totalbridge = amount
        bridgedmin = web3.from_wei(amountin, 'ether')
        ...
        print(f'Processing Bridge {bridgedmin} ETH BASE TO ETH BASE')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Bridge {bridgedmin} ETH BASE TO ETH BASE Success!')
        print(f'TX-ID : {str(web3.to_hex(tx_hash))}')
