...
def Executor(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        bridgeaddr = web3.to_checksum_address('0x1D5FD4ed9bDdCCF5A74718B556E9d15743cB26A2') #for blast
        ...
        print(f'Processing Executor Bridge {bridgedmin} From ETH BLAST TO ETH BLAST')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bridge {bridgedmin} From ETH BLAST TO ETH BLAST Success!')
...
def Get_Estimate(sender, key, amount):
    ...
    data = {
        ...
        "toAsset": "eth",
        "fromChain": "blss",
        "toChain": "blss", #arbt (arb) | bssp (base) | blss (blast) | opsp (op)
...
def BidExecute(sender, key, amount, amountin, orderid, txhash):
    try:
        ...
        print(f'Processing Bid Executor Bridge {bridgedmin} From ETH BLAST TO ETH BLAST')
        web3_brn.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Executor Bid Bridge {bridgedmin} From ETH BLAST TO ETH BLAST Success!')
...
def BLAST_BLAST(sender, key, amount, amountin):
    try:
        ...
        print(f'Processing Bridge {bridgedmin} ETH BLAST TO ETH BLAST')
        web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f'Bridge {bridgedmin} ETH BLAST TO ETH BLAST Success!')
        print(f'TX-ID : {str(web3.to_hex(tx_hash))}')
